...
def build_3d_grid(min_xyz,
  ...
  Args:
    ...
    worldspace_t_opengl: An optional 4x4 transformation matrix that maps the
      native coordinate space of the NeRF model to an OpenGL coordinat  e system,
...
def render_voxel_block(model, mip_lvl, device, render_viewpoints_kwargs, block_coordinates_world,
  ...
  origins = np.copy(block_coordinates_world.reshape((-1, 3)))
  origins = np.resize(origins, (rounded_num_rays, 3))
  origins = origins.reshape((-1, batch_size, 3))

  rgb_and_features = np.zeros(
      (origins.shape[0], origins.shape[1], channels),
      dtype=output_dtype)
  sigma = np.zeros_like(rgb_and_features[:, :, 0])
  alpha = np.zeros_like(rgb_and_features[:, :, 0])
  for i in range(origins.shape[0]):
    batch_origins = origins[i]
    batch_origins = batch_origins.reshape(-1, 3)

    host_batch_origins = torch.from_numpy(batch_origins).to(device)
    with torch.no_grad():
        output = model.bake(host_batch_origins.float(), mip_lvl,  **render_viewpoints_kwargs)
    batch_rgb, batch_sigma, batch_alpha =\
        output['raw_rgb'].detach().cpu().numpy(), output['raw_sigma'].detach().cpu().numpy(), output['raw_alpha'].detach().cpu().numpy()

    rgb_and_features[i] = np.array(
        batch_rgb, dtype=output_dtype).reshape(rgb_and_features[i].shape)
    sigma[i] = np.array(
        batch_sigma, dtype=output_dtype).reshape(sigma[i].shape)
    alpha[i] = np.array(
        batch_alpha, dtype=output_dtype).reshape(alpha[i].shape)
  ...
  alpha = alpha[:actual_num_rays]

  # alpha = 1.0 - np.exp(-sigma * 0.25)
...
def extract_3d_atlas(model, mip_size, device, render_viewpoints_kwargs, scene_params, render_params,
  ...
  """
  atlas_block_size = atlas_params['atlas_block_size'] #32
  min_xyz = scene_params['min_xyz']
  voxel_size = render_params['_voxel_size']

  data_block_size = atlas_params['_data_block_size'] #30
  ...
  def mip_factor(level):
      scale = 2 ** level
      return scale

  if mip_size > 0:
    atlas_mipmap = []
    atlas_world_coordinates_mipmaps = []
    atlas_valid_mask_mipmaps = []
    for l in range(mip_size):
      factor = mip_factor(l+1)
      assert atlas_slice_size % factor == 0
      assert atlas_slices % factor == 0
      atlas_mipmap.append(
        np.zeros((atlas_slice_size//factor,
                  atlas_slice_size//factor,
                  atlas_slices//factor,
                  num_channels + 1), dtype=atlas_dtype))
      atlas_world_coordinates_mipmaps.append(
        np.zeros((atlas_slice_size//factor,
                  atlas_slice_size//factor,
                  atlas_slices//factor, 3), dtype=atlas_dtype))
      atlas_valid_mask_mipmaps.append(
        np.zeros((atlas_slice_size//factor,
                  atlas_slice_size//factor,
                  atlas_slices//factor), dtype=atlas_dtype))
  else:
    atlas_mipmap = None
    ...
    for block_y in range(atlas_grid_size[1]):
      for block_z in range(atlas_grid_size[2]):
        ...
        if mip_size > 0:
          for l in range(mip_size):
            factor = mip_factor(l + 1)
            atlas_world_coordinates_temp = atlas_world_coordinates_mipmaps[l]
            atlas_valid_mask_temp = atlas_valid_mask_mipmaps[l]
            # Args:
            #     min_xyz: The minimum XYZ location of the grid.
            #     voxel_size: The side length of a voxel.
            #     grid_size: A numpy array containing the grid dimensions [H, W, D].
            assert atlas_x % factor == 0
            assert (atlas_x + atlas_block_size) % factor == 0
            assert atlas_y % factor == 0
            assert (atlas_y + atlas_block_size) % factor == 0
            assert atlas_z % factor == 0
            assert (atlas_z + atlas_block_size) % factor == 0
            assert (atlas_block_size_3d % factor == 0).all()

            atlas_world_coordinates_temp[
              atlas_x//factor:(atlas_x + atlas_block_size)//factor,
              atlas_y//factor:(atlas_y + atlas_block_size)//factor,
              atlas_z//factor:(atlas_z + atlas_block_size)//factor,
              0:3] = build_3d_grid(
                min_xyz + (atlas_min_voxel) * voxel_size,
                                   voxel_size * float(factor), atlas_block_size_3d//factor,
                                   scene_params['worldspace_T_opengl'])
            atlas_valid_mask_temp[atlas_x//factor:(atlas_x + atlas_block_size)//factor,
                                  atlas_y//factor:(atlas_y + atlas_block_size)//factor,
                                  atlas_z//factor:(atlas_z + atlas_block_size)//factor] = 1.0
  ...
  for _ in range(num_samples):
    ...
    for z in range(atlas.shape[2]): # [2048, 2048, 32, 8]
      atlas_rgb, atlas_alpha = render_voxel_block(
          model, 0, device, render_viewpoints_kwargs,
          atlas_world_coordinates[:, :, z:z + 1, :] , voxel_size,
          scene_params)
    ...
    if mip_size > 0:
      for l in range(mip_size):
        factor = mip_factor(l + 1)
        atlas_temp = atlas_mipmap[l]
        atlas_world_coordinates_temp = atlas_world_coordinates_mipmaps[l]
        for z in range(atlas_temp.shape[2]):
          atlas_rgb, atlas_alpha = render_voxel_block(
            model, l + 1, device, render_viewpoints_kwargs,
            atlas_world_coordinates_temp[:, :, z:z + 1, :],
            voxel_size * factor, scene_params)
          atlas_temp[:, :, z:z + 1, 0:num_channels] += atlas_rgb
          atlas_temp[:, :, z:z + 1, num_channels] += atlas_alpha
  ...
  if mip_size > 0:
    for l in range(mip_size):
      atlas_temp = atlas_mipmap[l]
      atlas_valid_mask_temp = atlas_valid_mask_mipmaps[l]
      atlas_temp /= num_samples
      atlas_temp[atlas_valid_mask_temp < 1.0] = 0.0

  return atlas, atlas_block_indices, atlas_mipmap #[2048, 2048, 32, 8], [23,34,24]