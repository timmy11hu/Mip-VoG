...
def crop_alpha_grid(render_params,
  ...
  print("minmax in voxels", min_in_voxels, max_in_voxels)
  ...
  print("minmax in world", min_in_world, max_in_world)
    ...
    for block_y in range(atlas_params['_atlas_grid_size'][1]):
      for block_z in range(atlas_params['_atlas_grid_size'][2]):
        ...
        if block_values.shape[-1] == 0:
            print("size parameter illed")
        atlas_grid_maximum[block_x, block_y, block_z] = np.max(block_values)
