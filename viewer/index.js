...
let gMip = 3; // level of highest mipmap, 0 for not mipmapping
let gFrameWidth = 800;
let gFrameHeight = 800;
...
const rayMarchVertexShader = `
  ...
  varying vec3 dxDirection;
  varying vec3 dyDirection;
  ...
  void main() {
    ...
    vOrigin = nearPoint.xyz / nearPoint.w; // world space
    vDirection = normalize(farPoint.xyz / farPoint.w - vOrigin); //world space

    // neighbors coordinates
    vec4 nearPointX = world_T_clip * vec4(positionClip.x + 1.0/float(width), positionClip.y, -1.0, 1.0);
    vec4 farPointX = world_T_clip * vec4(positionClip.x + 1.0/float(width), positionClip.y, 1.0, 1.0);
    dxDirection = normalize(farPointX.xyz / farPointX.w - vOrigin);

    vec4 nearPointY = world_T_clip * vec4(positionClip.x, positionClip.y + 1.0/float(height), -1.0, 1.0);
    vec4 farPointY = world_T_clip * vec4(positionClip.x, positionClip.y + 1.0/float(height), 1.0, 1.0);
    dyDirection = normalize(farPointY.xyz / farPointY.w - vOrigin);
  }
`;
...
const rayMarchFragmentShaderBody = `
  ...
  void main() {
    ...
    // dx dy
    mediump vec3 dxDirectionWorld = normalize(dxDirection);
    mediump vec3 dyDirectionWorld = normalize(dyDirection);
    ...
    // dx dy
    mediump vec3 dxDirectionGrid = dxDirectionWorld;
    mediump vec3 dyDirectionGrid = dyDirectionWorld;
    ...
    // dx dy
    mediump vec3 dxPosGrid = originGrid + dxDirectionGrid * t;
    mediump vec3 dyPosGrid = originGrid + dyDirectionGrid * t;
    ...
    while (step < maxStep && t < tMinMax.y && visibility > 1.0 / 255.0) {
      ...
      if (atlasBlockIndex.x > 254.0) {
        ...
        const float miniBlockSize = float(1 << skipMipLevel); // 4

        // Only fetch one byte at first, to conserve memory bandwidth in
        // empty space.
        // texelFetch on the other hand expects integer texel indices while  textureLod and texture can do trilinear fetch
        lowp float atlasAlpha = texelFetch(
          mapAlpha, ivec3(posAtlas / miniBlockSize), skipMipLevel).x; 

        if (atlasAlpha > 0.0) {
         if (mip > 0 && mipLod <= 0.0) {
            mediump float dxNorm = length(dxPosGrid - posGrid);
            mediump float dyNorm = length(dyPosGrid - posGrid);
            mipLod = clamp(max(dxNorm, dyNorm) / 3.0, 0.0, float(mip));
          }

          // OK, we hit something, do a proper trilinear fetch at high res.
          mediump vec3 atlasUvw = posAtlas / atlasSize; 
          atlasAlpha = textureLod(mapAlpha, atlasUvw, mipLod).x;
          ...
          if (atlasAlpha > 0.5 / 255.0) {
            ...
            atlasRgba.rgb = textureLod(mapColor, atlasUvw, mipLod).rgb;
            if (displayMode != DISPLAY_DIFFUSE) {
              lowp vec4 atlasFeatures = textureLod(mapFeatures, atlasUvw, mipLod);
     ...
     if (mip > 0 && mipLod <= 0.0) {
         dxPosGrid = originGrid + dxDirectionGrid * t;
         dyPosGrid = originGrid + dyDirectionGrid * t;
      }
      if (t > tBlockMinMax.y) {
...
function createRayMarchMaterial(
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      'mip': {'value' : gMip},
      'width': {'value' : gFrameWidth},
      'height': {'value' : gFrameHeight},
      ...
      'world_T_camera': {'value': new THREE.Matrix4()},
      'camera_T_clip': {'value': new THREE.Matrix4()},
 ...
 * @param {number} level
 */
function computeMipFactor(level){
  return (1 << level);
}
...
function loadSplitVolumeTexturePNG(
  ...
  for (let i = 0; i < num_slices; i++) {
    ...
    uploadPromises[count++] = new Promise(function(resolve, reject) {
          ...
          .then(values => {
            ...
            for (let z = 0; z < slice_depth; ++z) {
              ...
              //texSubImage3D(target, level, xoffset, yoffset, zoffset,
                // width, height, depth, format, type, srcData, srcOffset)
              gl.texSubImage3D(
                  gl.TEXTURE_3D, 0, 0, y, z + i * slice_depth, // z(current) + i * slice_depth (which png)
  ...
  if(gMip > 0) {
    for (let l = 1; l <= gMip; l++){
      let mipmap_factor = computeMipFactor(l);
      let num_slice_mip = parseInt(num_slices/mipmap_factor);
      let volume_width_mip = parseInt(volume_width/mipmap_factor);
      let volume_height_mip = parseInt(volume_height/mipmap_factor);
      for (let i = 0; i < num_slice_mip; i++) {
        let rgbaUrl = url + '_' + digits(l, 1) + digits(i, 2) + '.png';
        let rgbaPromise = loadPNG(rgbaUrl);
        rgbaPromise = rgbaPromise.then(data => {
          on_update();
          return data;
        ...
        uploadPromises[count++] = new Promise(function(resolve, reject) {
          Promise.all([rgbaPromise, i])
              .then(values => {
                ...
                let i = values[1];
                let rgbPixels = new Uint8Array(
                    volume_width_mip * volume_height_mip * slice_depth * 3);
                let alphaPixels = new Uint8Array(
                    volume_width_mip * volume_height_mip * slice_depth * 1);
                for (let j = 0; j < volume_width_mip * volume_height_mip * slice_depth;
                     j++) {
                  rgbPixels[j * 3 + 0] = rgbaPixels[j * 4 + 0];
                  rgbPixels[j * 3 + 1] = rgbaPixels[j * 4 + 1];
                  rgbPixels[j * 3 + 2] = rgbaPixels[j * 4 + 2];
                ...
                gl.bindTexture(
                    gl.TEXTURE_3D, rgbTextureProperties['__webglTexture']);
                for (let z = 0; z < slice_depth; ++z) {
                  for (let y = 0; y < volume_height_mip; ++y) {
                    gl.texSubImage3D(
                        gl.TEXTURE_3D, l, 0, y, z + i * slice_depth,
                        volume_width_mip, 1, 1, gl.RGB, gl.UNSIGNED_BYTE,
                        rgbPixels, 3 * volume_width_mip * (y + volume_height_mip * z));
                  }
                }
                gl.bindTexture(
                    gl.TEXTURE_3D, alphaTextureProperties['__webglTexture']);
                for (let z = 0; z < slice_depth; ++z) {
                  for (let y = 0; y < volume_height_mip; ++y) {
                    gl.texSubImage3D(
                        gl.TEXTURE_3D, l, 0, y, z + i * slice_depth,
                        volume_width_mip, 1, 1, gl.RED, gl.UNSIGNED_BYTE,
                        alphaPixels, volume_width_mip * (y + volume_height_mip * z));
                  }
                }
                gl.bindTexture(gl.TEXTURE_3D, oldTexture);
                resolve(texture_rgb);
              ...
              .then(values => {
                ...
                for (let z = 0; z < slice_depth; ++z) {
                  for (let y = 0; y < volume_height_mip; ++y) {
                    gl.texSubImage3D(
                        gl.TEXTURE_3D, l, 0, y, z + i * slice_depth,
                        volume_width_mip, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE,
                        rgbaImage, 4 * volume_width_mip * (y + volume_height_mip * z));
                  }
                }
                gl.bindTexture(gl.TEXTURE_3D, oldTexture);
                resolve(texture);
...
function loadScene(dirUrl, width, height) {
  ...
  initializedPromise.then(values => {
    ...
    requestAnimationFrame(render); //Async call
    ...
    rgbVolumeTexture.minFilter = rgbVolumeTexture.magFilter = THREE.LinearFilter;
    ...
    if (!gSceneParams['diffuse']) {
      ...
      featureVolumeTexture.magFilter = featureVolumeTexture.minFilter = THREE.LinearFilter;
    ...
    if (gMip > 0) {
      console.log('using mip when rendering')
      rgbVolumeTexture.generateMipmaps = true;
      rgbVolumeTexture.minFilter = THREE.LinearMipmapLinearFilter;
      if (!gSceneParams['diffuse']) {
        featureVolumeTexture.generateMipmaps = true;
        featureVolumeTexture.minFilter = THREE.LinearMipmapLinearFilter;
      }
      alphaVolumeTexture.minFilter = THREE.LinearMipmapLinearFilter;
...
function initFromParameters() {
  ...
  let width = gFrameWidth;
  let height = gFrameHeight;
  if (size) {
...
function loadOnFirstFrame() {
  ...
  if (gSceneParams['ndc']) {
    ...
    gCamera.position.z = -8.0; // orginal-4
  }
  ...
  let rgbVolumeTexturePromise = loadSplitVolumeTexturePNG(
      alphaVolumeTexture, rgbVolumeTexture,
  ...
  allTexturesPromise.then(texture => {
    ...
    if (gMip <= 1){
      const rgbTextureProperties =
            gRenderer['properties'].get(rgbVolumeTexture);
      let gl = gRenderer.getContext();
      let oldTexture = gl.getParameter(gl.TEXTURE_BINDING_3D);
      gl.bindTexture(gl.TEXTURE_3D, rgbTextureProperties['__webglTexture']);
      gl.generateMipmap(gl.TEXTURE_3D);
      const rgbMip1 = new Uint8Array(
          1024 * 1024 * 16 * 3).fill(0);
      for (let z = 0; z < 16; ++z) {
        for (let y = 0; y < 1024; ++y) {
          gl.texSubImage3D(
              gl.TEXTURE_3D, 1, 0, y, z,
              1024, 1, 1, gl.RGB, gl.UNSIGNED_BYTE,
              rgbMip1, 3 * 1024 * (y + 1024 * z));
        }
      }
      gl.bindTexture(gl.TEXTURE_3D, oldTexture);
    }
